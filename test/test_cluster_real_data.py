...
def main():
    ...
    # --- Clustering bilanciato per risorse ---
    print("mando resource log")
    input_request.update({
        "resources": resource_log_list,
        "max_number_nodes": 780
    })
    json_resource = requests.post("http://localhost:4567/api/communities/3", json=input_request).json()
    print("salvo resource log")
    ...
    print("Arrivato il risultato originale")
    ...
    for community in json_original.get("communities", []):
        for member in community.get("members", []):
            ...
            if node_id in nodes:
                ...
                member["labels"]["x"] = x
                member["labels"]["y"] = y
    print("Salvo il risultato originale")
    ...
    print("Salvato il risultato originale")
