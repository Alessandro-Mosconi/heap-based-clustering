...
def plot_clusters(cluster_data, centroids, nodes, route_matrix, title):
    ...
    for cluster_idx, data in enumerate(cluster_data):
        ...
        for node_id in data["shared_nodes"]:
            ...
            wedge1 = Wedge(shared_coord, 1, 0, 180, facecolor=color1, edgecolor='black')
            wedge2 = Wedge(shared_coord, 1, 180, 360, facecolor=color2, edgecolor='black')
            ax.add_patch(wedge1)
            ax.add_patch(wedge2)
    ...
    plt.savefig(snakecase(title) + '_random.png', format="png", dpi=300)
    plt.show()
...
save_cluster_data("resoconto_random.json", cluster_data, nodes)
